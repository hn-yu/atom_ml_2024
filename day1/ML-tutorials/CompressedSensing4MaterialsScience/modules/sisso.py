...
class SissoRegressor(object):
    ...
    def fit(self, D, P):
        ...
        for i_iter in range(self.n_nonzero_coefs):
            ...
            indices_n_closest, best_projection_score = self._sis(D, Residual) # 选出最好的n_features_per_sis_iter个特征
            self.sis_selected_indices.append( indices_n_closest ) #[[1,3,4], [2,5,6]]
    ...
    def _l0_regularization(self, D, P, list_of_sis_indices):
        ...
        if self.all_l0_combinations:
            combinations_generator = combinations(np.concatenate(list_of_sis_indices), len(list_of_sis_indices)) #求所有可能的组合
        else:
            combinations_generator = product(*list_of_sis_indices) # list_of_sis_indices各元素作为参数求张量集，即每个SIS步骤中都会确定一个
